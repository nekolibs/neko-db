...
- **Conditional defs — optional `enabled({ db })` predicate** (sync/async) on Pull AND Push: false → skip that cycle BEFORE collect/pull runs, cursor NOT advanced (`markRun`, not `setCursor`) — so a push's dirty rows stay dirty and a pull resumes from the same server cursor once re-enabled. Declarative per-def gate, e.g. `enabled: () => !!getSession()?.user?.id` (push only while logged in; a guest's local edits push up the first cycle after login, nothing lost). `collect`→null also skips safely, but `enabled` is declarative, reused on pulls, and short-circuits before the SQLite read.
- **Write-trigger scheduling (Push only)**: `autoPush` (default true) / `debounce` (seconds). `autoPush: false` opts a def out of the per-write debounced push — rows still go up on syncOnStart/interval/foreground/reconnect + manual `push()`, just not per keystroke (chatty edit flows like a workout logged set-by-set; push explicitly on finish). `debounce` overrides the global `debouncePush` for that def. Caveat: an `autoPush: false` def is still pushed when a downstream `dependsOn` it and gets debounced (dependency closure drags it in — ordering wins). Mapping via `pushDefsForModel(name)` in registry.js.
...
- **Wiring**: `<NekoDB>` provides DB + bridges + `_sync_cursors` only (no sync props). Sync is a separate opt-in provider: mount `<SyncProvider pushes={pushes} pulls={pulls} enabled={bool} config={{ interval, debouncePush, cooldown }} />` where it belongs (e.g. inside the auth tree, `enabled` gated on an active session — toggling `enabled` starts/stops triggers without remounting; defs register once). Triggers: syncOnStart, AppState active, NetInfo reconnect (optional peer), foreground interval (default 15min), per-def debounced push on synced-model emits (each def its own timer + `dependsOn` closure; `autoPush`/`debounce` tune it), cooldown (default 30s) gating rapid full cycles. Hooks: `useSyncStatus`, `useSync`. Cursors in lib-owned `_sync_cursors` (created in NekoDB onInit; `resetDatabase` drops it → full resync).
